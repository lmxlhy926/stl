...
template <typename T> ssize_t handle_EINTR(T fn) {
  ...
  while (true) {
    res = fn();
    if (res < 0 && errno == EINTR) { continue; }    //中断导致的返回
    break;
  }
  return res;
}
...
process_server_socket(socket_t sock, size_t keep_alive_max_count,
  ...
  return process_server_socket_core(
      sock, keep_alive_max_count, keep_alive_timeout_sec,  //检测到有数据，则执行
...
bool Server::listen_internal() {
  ...
  {
    std::unique_ptr<TaskQueue> task_queue(new_task_queue());    //创建线程池

    while (svr_sock_ != INVALID_SOCKET) {   //处理客户端请求
#ifndef _WIN32
...
#endif
      socket_t sock = accept(svr_sock_, nullptr, nullptr);  //和客户端建立连接的socket
...
#else
      task_queue->enqueue([=]() { process_and_close_socket(sock); });   //处理相应客户端的请求
#endif
    }
