...
**字节序**：字节序的转换就是改变整数的存储方式。在网络中传输一个多字节整数时，总是先传输低地址字节，再传输高地址字节。对端也是先接收低地址字节，再接收高地址字节。因此必须约定低地址字节代表的是整数的低有效字节还是高有效字节。TCP/IP中采取网络字节序，即低地址代表的是高有效字节。约定了存储方式，解析方式也就随之确定。
