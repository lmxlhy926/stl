...
bool CharReaderBuilder::validate(Json::Value* invalid) const
{
  ...
  if (!invalid) invalid = &my_invalid;  // so we do not need to target for NULL
...
//add by wjg 20190917 ---parse utf8 file has BOM header
  if( ((begin[0]&0xFF)==0xEF) &&
      ((begin[1]&0xFF)==0xBB) &&
      ((begin[2]&0xFF)==0xBF) )
  {
      begin += 3;
  }
//end add by wjg 20190917
