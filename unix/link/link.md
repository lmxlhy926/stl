...
基本思路是当创建可执行文件时，静态执行一些链接，然后在程序加载时，动态完成链接过程。可执行文件没有复制共享库的代码和数据节，反之，链接器复制了一些重定位和符号表信息，它们使得运行时可以解析对.so中代码和数据的引用。当加载器加载部分链接的可执行文件prog21时，它注意到prog21包含一个.interp节，这一节包含动态链接器的路径名，动态链接器本身就是一个共享目标(如在Linux系统上的ld-linux.so)。
